...
#---------------Random-Forsest--------------------#
#-------------------------------------------------#
def randForest(X, y, K=10, max_features=None):
	# typical K = 10, 30, 100, the higher, the better!
	rf = RandomForestClassifier(n_estimators=K, criterion='gini', max_depth=None, min_samples_split=2, min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features=max_features, max_leaf_nodes=None, min_impurity_split=1e-07, bootstrap=True, oob_score=False, verbose=1, n_jobs=1)
	rf.fit(X, y)
	return rf
